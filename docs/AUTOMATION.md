...
Fourseasquant 使用 macOS LaunchAgent（用户登录后的后台启动代理）每分钟唤醒一次轻量检查，再由应用按设置面板保存的北京时间决定是否运行每日任务。默认时间为 16:30，因此系统时区变化不会改变北京时间口径。定时和网站内手动运行都会先更新 AKShare 股票与五个指数 K 线，计算并暂存全市场个股技术评分，再进入策略、校验和事务发布。技术评分批次与策略快照只会在最后一步同时激活；任何一部分不完整都不会覆盖上一版结果。
