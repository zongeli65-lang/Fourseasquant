...
当前只有数据模型和测试夹具，尚无用户核心策略到该模型的正式适配器，也没有真实历史趋势快照。新 Agent 不得用均线、指数涨跌或其他临时规则替用户策略生成趋势。
...
1. 用户核心策略代码尚未提供，无法生成正式 `MarketTrendSnapshot`；
2. 尚无真实趋势历史的存储、查询和版本管理；
3. 基本面模块到正式 `SectorMembershipSnapshot` 的生产发布适配器尚未完成；
4. 尚无把三类输入组装成 `ElectionDay` 的生产编排服务；
5. 每日 16:30 流水线尚未运行和原子发布板块选举；
6. 尚无板块结果的生产查询接口；
7. 尚无日期范围板块回放入口；
8. 网站尚无真实板块技术龙头排行榜、热力图和候选详情；
9. 尚未用真实成员历史和真实策略趋势完成全量验收。
